...
func init() {
	http.HandleFunc("/msx/"+pthMarks, func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "POST" {
			...
			ls, id, pl := webmarks(), r.FormValue("id"), mediaList(r, "", "{ico:msx-white:bookmarks}", "movie", []plistObj{{"key": "red", "label": "{dic:Remove|Remove}", "action": "execute:http://" + r.Host + r.URL.Path, "data": "{context:id}"}}, true, false, false)
